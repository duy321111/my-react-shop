        ...
        {selectedBrands.map((id) => {
          ...
          return (
            <div key={id} className={styles.chip}>
              {brand?.name}
              <span className={styles.removeChip} onClick={() => handleRemove(id)}>
                ×
              </span>
            </div>
          );
              ...
              {categories.map((c) => (
                <div key={c._id} className={styles.categoryItem}>
                  ...
                  {c.brands && c.brands.length > 0 && (
                    <p>Brands: {c.brands.map((b) => b.name).join(", ")}</p>
                  )}
                  {c.description && <p>{c.description}</p>}
                  {c.image && <img src={c.image} alt={c.name} className={styles.categoryImg} />}
                </div>
              ))}
            </div>
          </div>
